...
echo $($PSQL "SELECT teams.name FROM teams INNER JOIN games ON teams.team_id = games.winner_id WHERE games.round = 'Final' AND games.year = 2018")
...
echo  $($PSQL "SELECT games.year, teams.name FROM games INNER JOIN teams ON games.winner_id = teams.team_id WHERE round = 'Final' AND year = 2014 OR round = 'Final' and year = 2018 ORDER BY year")
